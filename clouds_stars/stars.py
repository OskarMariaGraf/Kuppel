...
# um das Programm zu verstehen sollte man mit Python und insbesondere mit NumPy
# vertraut sein. Wahrscheinlich fängt man am besten mit der Methode num_stars
# an und schaut sich dann die von num_stars aufgerufenen Methode an
...
#    die Helligkeit des Pixels wenn man nach der durchschnittlichen Helligkeit
#    des Bildes normalisiert
...
def find_points(gray: np.array, db=DB_MIN_POINT, s=VERSCHIEBUNG) -> List[Pixel]:
    """"
    gibt eine Liste von Pixel zurück, die viel heller als ihre unmittelbare
    Umgebung sind
    ...
    # wir verschieben das Bild/Matrix um s pixel und ziehen 1/4 vom
    # urpsrünglichen Bild ab
    delta = gnorm.copy()
    delta[s:, :] -= gnorm[:-s, :] / 4
    delta[:-s, :] -= gnorm[s:, :] / 4
    delta[:, s:] -= gnorm[:, :-s] / 4
...
def filter_stars(stars: List[Pixel], n=3, ab_min=FILTER_AB_MIN,
                 rb_min=FILTER_RB_MIN,
...
def clouded(gray: np.array) -> float:
    ...
    """
    x2, y2 = gray.shape[0] // 2, gray.shape[
        1] // 2  # Koordinaten des Mittelpunkts

    # Das bild (ein 2D-array aus Graustufenwerten) wird in 4 Quadranten
    # eingeteilt
    a, b, c, d = (
        num_stars(gray[:x2 + 3, :y2 + 3]), num_stars(gray[x2 - 3:, :y2 + 3]),
        num_stars(gray[:x2 + 3, y2 - 3:]), num_stars(gray[x2 - 3:, y2 - 3:]))

    # für jeden Quadranten, in dem kein Stern sichtbar ist beträgt de
    # cloudcover-Wert 0.25
    return 1 - ((a > 0) +
                (b > 0) +
                (c > 0) +
                (d > 0)) \
